...
def suggest_mitigation(
    ...
    try:
        ...
        - Regla Violada: {getattr(rule, 'name', 'Desconocida')}
        - Nivel de Severidad: {getattr(rule, 'severity', 'Desconocida')}
        - Score de Riesgo: {conflict.risk_score}/100
        - Usuario SAP: {getattr(user, 'user_id', 'Desconocido')} ({getattr(user, 'full_name', 'Sin Nombre')})
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=400, detail=f"Error interno AI: {str(e)}")
...
def generate_audit_summary(
    ...
    try:
        ...
        rule_counts = {}
        for c in conflicts:
            rname = c.rule.name if getattr(c, 'rule', None) else "Desconocida"
